...
export interface IGameLoopClockState {
  last: null | number;
  timer: number;
  threshold: number;
}
...
export type IRenderCallback = (delta: number, currentFrame: number) => void;
...
const _GameLoopClock: IGameLoopClockState = {
  last: null,
  ...
  run: function (): void {
    ...
    if (_GameLoopClock.timer > _GameLoopClock.threshold) {
      ...
      for (let i = 0; i < _GameLoopState.layers.length; i++) {
        ...
        layer.update(_GameLoopClock.timer, _FpsTracker.counter);
        layer.render && layer.render(_GameLoopClock.timer, _FpsTracker.counter);
      }
